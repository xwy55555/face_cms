  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        if (pathToRegexp('/index/content_manage/carousel_manage/carousels/:id/edit').test(location.pathname)) {
    ...
    *initEdit({ payload }, { call, put }) {
      ...
      if (data) {
        ...
        if (data.code === projectConstant.CODE_SUCCESS) {
          yield put({ type: 'setValue', payload: { curAction: 'edit', curActionName: '编辑轮播图', curRecord: data.data, actionBarVisible: false } });
        }
      }
    },
