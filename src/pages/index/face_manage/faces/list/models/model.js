...
import { getPageList, remove, changeOrderBy } from '../../services/service';
...
export default {
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname === '/index/face_manage/faces/list') {
          dispatch({
            type: 'initList',
            payload: location,
          });
  ...
  reducers: {
    ...
    deleteSuccess(state, action) {
      ...
      const newList = state.dataSource.filter(face => face.id !== id);
